...
function getChannel(): BroadcastChannel | null {
  if (!IS_WEB || typeof BroadcastChannel === 'undefined') return null;
...
export function startIdleAutoLock({ isStaff, onLock }: AutoLockOpts): () => void {
  ...
  const events = ['mousemove', 'keydown', 'touchstart', 'scroll', 'visibilitychange'] as const;
...
export async function hardSignOut(reason?: string) {
  ...
  stopIdleAutoLock();
  broadcastSignOut();
  if (reason && IS_WEB && typeof window !== 'undefined') {
    try { getBrowserStorage()?.setItem('cofkans_signout_reason', reason); } catch { /* ignore */ }
  }
}
