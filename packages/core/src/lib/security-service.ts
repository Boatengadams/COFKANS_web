...
export async function recordFailedLogin(email: string, ipAddress?: string): Promise<{
  ...
  try {
    const call = httpsCallable<
      { email: string; action: string; ipAddress?: string },
      { isLocked: boolean; attemptsRemaining: number; lockedUntil?: string; delayMs: number }
    >(functions, 'recordLoginAttempt');
...
export async function clearFailedLogins(email: string) {
  try {
    const call = httpsCallable<{ email: string; action: string }, { ok: true }>(
      functions,
      'recordLoginAttempt',
    );
    await call({ email: email.toLowerCase(), action: 'clear' });
...
export async function checkAccountLocked(email: string): Promise<boolean> {
  try {
    const call = httpsCallable<
      ...
      { isLocked: boolean }
    >(functions, 'recordLoginAttempt');
    const result = await call({ email: email.toLowerCase(), action: 'status' });
    return Boolean(result.data.isLocked);
  } catch (error) {
  ...
  try {
    ...
    if (trackerDoc.exists()) {
      ...
      if (now < windowEnd) {
        ...
        const maxCount = 'MAX_PER_MINUTE' in config ? config.MAX_PER_MINUTE : config.MAX_PER_HOUR;
