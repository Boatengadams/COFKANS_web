...
import { Send, Sparkles, User as UserIcon, ShieldAlert, ArrowUp, Headphones, Clock, Star, CheckCircle2, Phone, Mic, MicOff, Lock } from 'lucide-react';
import { useSupportStore } from '@/stores/support-store';
import { statusLabel, BUSINESS_HOURS_HUMAN, supportPhone } from '@/lib/support/business-hours';
    ...
    <div className={`flex flex-col ${heightClass} bg-card text-foreground`}>
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-bold text-sm leading-tight">
            {session.assignedAgentName ? session.assignedAgentName : 'Vanessa'}
          </div>
          ...
          <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden border-t border-border bg-muted/30">
            <div className="p-3 space-y-2">
              <div className="text-xs font-bold flex items-center gap-1.5">
                ...
                {phone.afterHours ? 'After-hours line' : 'Customer service'}
              </div>
              <a
                href={`tel:${phone.tel}`}
                className="block w-full text-center px-3 py-2.5 rounded-xl bg-primary text-primary-foreground text-base font-bold tracking-wide hover:opacity-90"
              >
                {phone.display}
              </a>
              <p className="text-[11px] text-muted-foreground leading-snug">
                {phone.afterHours
                  ? `We're closed right now (open ${BUSINESS_HOURS_HUMAN}). This US line handles after-hours calls.`
                  : `Open now · ${BUSINESS_HOURS_HUMAN}. Tap to call our Ghana line.`}
              </p>
              <button onClick={() => setShowCall(false)} className="px-2.5 py-1 rounded-lg hover:bg-muted text-[11px] font-semibold">Close</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="border-t border-border p-3 space-y-2">
        ...
        <div className="flex items-center justify-between gap-2 text-[11px]">
          {session.status === 'ai' ? (
            <button onClick={() => { setShowCall(true); escalate(session.id, 'Customer requested'); }} className="text-muted-foreground hover:text-foreground font-semibold flex items-center gap-1">
              <Headphones className="w-3 h-3" /> Talk to a human
            </button>
          ) : <span />}
          <button onClick={() => setShowCall(v => !v)} className="text-muted-foreground hover:text-foreground font-semibold flex items-center gap-1">
            <Phone className="w-3 h-3" /> Call customer service
          </button>
        </div>
      </div>
    </div>
