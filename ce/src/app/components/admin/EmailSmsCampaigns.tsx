...
export function EmailSmsCampaigns() {
  ...
  const submitEmail = async () => {
    ...
    const newCampaign: EmailCampaign = {
      ...
      delivered: Math.floor(recipients.length * 0.95), // Mock: 95% delivery rate
      failed: Math.ceil(recipients.length * 0.05), // Mock: 5% failure rate
      status: 'completed',
      sentAt: Date.now(),
      reports: recipients.map((r) => ({
        email: r.customerId + '@example.com', // Mock email
        status: Math.random() > 0.05 ? 'delivered' : 'failed',
        reason: Math.random() > 0.05 ? undefined : 'Invalid email address',
      })),
    };
    ...
    setEmailContent('');
    setEmailHtml('');
    toast.success('Email campaign sent successfully! (Note: This is a demo - configure email service for production)');
  };
        ...
        {campaignType === 'email' && (
          <div className="bg-amber-500/10 border border-amber-200 rounded-xl p-3 flex items-start gap-2">
            ...
            <div className="text-xs text-amber-700">
              ...
              <p>Email sending is currently in demo mode. Configure your email service (SendGrid, AWS SES, etc.) for production use.</p>
            </div>
          </div>
        )}
      </div>
                  ...
                  >
                    ...
                    <div className="text-right hidden sm:block">
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        {isSms ? `GH₵${c.cost?.toFixed(2)}` : 'Free (demo)'}
                      </p>
                    </div>
                  </button>
                  <AnimatePresence>
