...
const PICKUP_STEPS: { key: string; label: string }[] = [
  { key: 'processing', label: 'Received' },
...
function OrderTrackingStrip({ order }: { order: OrderWithId }) {
  const steps = order.fulfillmentType === 'pickup' ? PICKUP_STEPS : DELIVERY_STEPS;
  const currentIdx = steps.findIndex(s => s.key === order.status);
  return (
    <div className="mb-4 p-3 rounded-xl border-2 border-border bg-background">
      <div className="flex items-center justify-between mb-2 text-xs">
        <span className="font-bold uppercase tracking-wider text-muted-foreground">
          {order.fulfillmentType === 'pickup' ? 'Pickup' : 'Delivery'}
          {order.branchSlug && ` · ${order.branchSlug}`}
        </span>
        {order.scheduledDate && (
          <span className="text-muted-foreground">Scheduled: {order.scheduledDate}</span>
        )}
      </div>
        ...
        {steps.map((s, i) => {
          ...
          return (
            <div key={s.key} className="flex-1 flex items-center gap-1">
              <div className={`flex-1 h-1.5 rounded-full ${done ? 'bg-primary' : 'bg-muted'}`} />
              <div className={`text-[10px] whitespace-nowrap ${active ? 'font-bold text-primary' : done ? 'text-foreground' : 'text-muted-foreground'}`}>
                {s.label}
              </div>
            </div>
          );
      ...
      {order.driverName && order.status === 'shipped' && (
        <p className="text-xs text-muted-foreground mt-2">
          Rider on the way: <span className="font-semibold text-foreground">{order.driverName}</span>
        </p>
      )}
    </div>
  );
...
export function MyOrders() {
  ...
  useEffect(() => {
    ...
    if (DEMO_MODE) { setOrders([]); setLoading(false); return; }

    // Live subscription so customers see status changes (packed → ready →
    // out for delivery → delivered) without refreshing the page.
    ...
    const unsub = onSnapshot(
      ordersQuery,
      snap => {
        setOrders(snap.docs.map(d => ({ id: d.id, ...(d.data() as Omit<OrderWithId, 'id'>) })));
        setLoading(false);
      },
      err => {
        ...
        <div className="space-y-4">
          {filteredOrders.map((order) => (
            ...
            >
              ...
              {/* Live tracking strip */}
              <OrderTrackingStrip order={order} />
