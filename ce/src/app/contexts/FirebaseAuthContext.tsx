...
export function FirebaseAuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      // Safety net: never let the loading state get stuck if a downstream
      // Firestore/network call hangs — surface as unauthenticated after 10s.
      const loadingWatchdog = setTimeout(() => {
        console.warn('Auth loading watchdog fired — forcing isLoading=false');
        setIsLoading(false);
      }, 10000);
