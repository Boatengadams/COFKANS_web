...
import { getStaffAccount } from '../../lib/staff';
import { setDemoRole, setSessionUser, type DemoRole } from '../../lib/demo-mode';
...
function RealAuthProvider({ children }: { children: ReactNode }) {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (firebaseUser) {
        try {
          ...
          // Sliding-window session: 30 days idle for customers, 7 days for
          // staff/developer. Each restore bumps the timestamp so an active
          // user effectively never gets logged out.
          const isStaff = !!(firestoreUser && (
            firestoreUser.role === 'admin' ||
            firestoreUser.role === 'technician' ||
            firestoreUser.role === 'driver' ||
            firestoreUser.isDeveloper
          ));
          ...
          if (firestoreUser && (firestoreUser.role === 'admin' || firestoreUser.isDeveloper)) {
            try {
  ...
  // Sync Firebase auth state to demo/session storage for role-based routing
  useEffect(() => {
    let cancelled = false;
    ...
    (async () => {
      try {
        ...
        let activeRole: DemoRole = 'guest';
        let staffRole: 'branch_manager' | 'rider' | 'front_desk' | 'developer' | undefined;
        let branchSlug: string | undefined;

        if (staff) {
          branchSlug = staff.branchSlug || undefined;
          if (staff.role === 'branch_manager' || staff.role === 'manager') {
            activeRole = 'manager';
            staffRole = 'branch_manager';
          } else if (staff.role === 'front_desk') {
            activeRole = 'front_desk';
            staffRole = 'front_desk';
          } else if (staff.role === 'rider' || staff.role === 'driver') {
            activeRole = 'driver';
            staffRole = 'rider';
          } else if (staff.role === 'developer') {
          ...
          if (isDev) {
            activeRole = 'developer';
            staffRole = 'developer';
          } else if (r === 'manager' || r === 'management_support' || r === 'admin') {
            activeRole = 'manager';
            staffRole = 'branch_manager';
        ...
        if (activeRole !== 'guest') {
          ...
          setSessionUser({
            ...
            isDeveloper: (user as any).isDeveloper === true,
            staffRole,
            branchSlug,
          });
        }
      } catch (err) {
        console.error('Error syncing staff session:', err);
      }
    })();
  ...
  /**
   * Staff accounts are provisioned server-side by managers/developers.
   * Customer signup remains OAuth-only.
   */
  const signUpWithEmail = async (
    ...
    void displayName;
    void role;
    const message = 'Staff signup is disabled. Ask a manager or developer to provision the account.';
    toast.error(message);
    setIsLoading(false);
  ...
  const signInWithEmail = async (email: string, password: string) => {
    ...
    try {
      ...
      if (firestoreUser) {
        // Initialize developer if needed
        if (isDeveloper(email) && !firestoreUser.isDeveloper) {
          await initializeDeveloper(firebaseUser.uid);
        }
...
// ===== DEMO / MOCK-BACKEND SWITCH =====
// When DEMO_MODE is on we transparently swap the whole auth layer for the
// localStorage-backed DemoAuthProvider (no Firebase calls). Consumers keep
// importing `useFirebaseAuth` / `FirebaseAuthProvider` from this file — the
// binding they get just points at the demo implementation instead. Flip
// DEMO_MODE in /src/lib/demo-mode.ts to restore the real backend.
import { DEMO_MODE, getBrowserStorage, IS_WEB } from '../../lib/demo-mode';
import { DemoAuthProvider, useDemoAuth } from './DemoAuthContext';

export const FirebaseAuthProvider = DEMO_MODE ? DemoAuthProvider : RealAuthProvider;
export const useFirebaseAuth = DEMO_MODE ? useDemoAuth : useRealAuth;
