...
export function ProtectedAdminRoute({
  ...
  // Not admin — allow isDeveloper users too (their role may be set to 'admin'
  // in Firestore but guard against any edge case where it isn't yet)
  ...
  // Super admin check — pass if isDeveloper flag is set OR email matches dev address
  if (requireDeveloper && !user.isDeveloper && !isDeveloper(user.email || '')) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-6">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
        ...
        >
          ...
          <h2 className="text-2xl font-bold mb-2">Developer Only</h2>
          <p className="text-muted-foreground">
            This action requires developer privileges.
          </p>
          <p className="text-sm text-muted-foreground mt-4">
