...
const BASE_PRODUCTS: Product[] = (csvProducts as Product[]).map(p => ({
  ...p,
...
export function ProductCatalog({ user, onRequireAuth, initialCategory = 'all' }: ProductCatalogProps) {
  ...
  // "Buy Now" — add the item then jump straight into the real checkout/payment
  // flow (App listens for `cofkans:checkout` and renders CheckoutPage → Paystack).
  const handleBuyNow = async (productId: string) => {
    if (!user) { onRequireAuth(); return; }
    const ok = await handleAddToCart(productId, 1);
    if (ok) {
      setShowCart(false);
    ...
    <div className="w-full space-y-8 relative">
      ...
      <AnimatePresence>
        ...
        {compareList.length > 0 && (
          <motion.button
            ...
            className="fixed bottom-28 right-3 sm:right-8 z-50 bg-secondary text-white rounded-full px-4 sm:px-6 py-3 sm:py-4 shadow-2xl flex items-center gap-2 sm:gap-3 font-bold cursor-pointer text-sm sm:text-base"
          >
        ...
        {cartItemCount > 0 && (
          <motion.button
            ...
            className="fixed bottom-8 right-3 sm:right-8 z-50 bg-primary text-white rounded-full px-4 sm:px-8 py-3 sm:py-5 shadow-2xl flex items-center gap-2 sm:gap-3 font-bold cursor-pointer text-sm sm:text-base"
            style={{
          ...
          {renderSequence.map(entry => entry.type === 'header' ? (
            ...
            <div
              ...
              className={`pc-card group relative bg-card/95 rounded-2xl overflow-hidden border border-border/70 shadow-[0_10px_35px_rgba(15,23,42,0.06)] hover:border-primary/35 hover:shadow-[0_22px_55px_rgba(15,23,42,0.14)] transition-all duration-500 cursor-pointer ${
                viewMode === 'list' ? 'flex gap-6' : 'flex h-full flex-col'
              }`}
            >
              {/* Product Image */}
              <div
      ...
      <ProductDetailModal
        ...
        onBuyNow={async (id, qty) => {
          const ok = await handleAddToCart(id, qty);
          if (ok) {
            setQuickViewProduct(null);
            setShowCart(false);
            window.dispatchEvent(new CustomEvent('cofkans:checkout'));
