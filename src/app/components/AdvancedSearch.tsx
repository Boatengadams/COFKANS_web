...
import { Search, X, SlidersHorizontal, Check, RotateCcw, BadgeCheck, Package, ArrowUpDown, Grid3x3, List } from 'lucide-react';
...
interface AdvancedSearchProps {
  ...
  // Optional controls merged into the filter panel (Sort / Pricing / View)
  sortBy?: string;
  onSortChange?: (value: string) => void;
  showTradePrice?: boolean;
  onToggleTradePrice?: () => void;
  viewMode?: 'grid' | 'list';
            ...
            >
              <div className="bg-card md:border-2 border-border md:rounded-2xl rounded-t-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] md:max-h-[75vh]">
                ...
                <div className="p-5 space-y-5 overflow-y-auto flex-1">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {SORT_OPTIONS.map(o => {
                          ...
                          return (
                            ...
                            >
                              {active ? <Check className="w-3 h-3" strokeWidth={3} /> : <ArrowUpDown className="w-3 h-3" strokeWidth={2.5} />}
                              <span>{o.label}</span>
                            </button>
                          );
                        })}
                  ...
                  {/* Pricing mode — applies instantly */}
                  {onToggleTradePrice && (
                    <Section title="Pricing">
                      <div className="flex gap-1.5">
                        <button
                          onClick={() => { if (showTradePrice) onToggleTradePrice(); }}
                          className={`flex-1 px-3 py-2 rounded-lg text-xs font-semibold border-2 transition-all flex items-center justify-center gap-1.5 ${
                            !showTradePrice
                              ? 'bg-foreground text-background border-foreground'
                              : 'bg-card text-foreground border-border hover:border-foreground/40 hover:bg-muted/50'
                          }`}
                        ...
                        <button
                          ...
                          className={`flex-1 px-3 py-2 rounded-lg text-xs font-semibold border-2 transition-all flex items-center justify-center gap-1.5 ${
                            showTradePrice
                              ? 'bg-gradient-to-r from-secondary to-primary text-white border-transparent shadow-md shadow-primary/30'
                              : 'bg-card text-foreground border-border hover:border-foreground/40 hover:bg-muted/50'
                          }`}
                        >
                          <span>🔑</span> Trade
                  ...
                  {onViewModeChange && (
                    <Section title="Product view">
                      <div className="grid grid-cols-2 gap-1.5 rounded-xl border border-border/70 bg-muted/40 p-1">
                        ...
                        >
                          <Grid3x3 className="w-3.5 h-3.5" strokeWidth={2.5} /> Grid
                        </button>
                        <button
