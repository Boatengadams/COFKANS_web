...
import cofkanFavIcon from '../imports/Cofkans_FavIcon.png';
...
const cofkanFavIconUrl = assetUrl(cofkanFavIcon);
...
// Renders the selected staff role's portal on top of the storefront (the
// preview mounts App.tsx directly, so portal routes live here, not in AppRouter).
const StaffPortalHost = lazy(() => import('./components/StaffPortalHost').then(m => ({ default: m.StaffPortalHost })));
...
const NavbarLogo = React.memo(function NavbarLogo({ onPress }: { onPress: () => void }) {
  return (
    <motion.button
      ...
      className="flex h-[80px] w-[210px] shrink-0 items-center relative group cursor-pointer"
      aria-label="Return to the Cofkans home page"
    >
        <div className="relative flex h-[80px] w-[210px] shrink-0 items-center px-3 py-1.5 rounded-xl bg-slate-950/95 shadow-[0_6px_18px_rgba(15,23,42,.12)] transition-all duration-300">
        <img
          src={cofkansLogoUrl}
          ...
          height={80}
          decoding="async"
          className="block h-[80px] w-[198px] shrink-0 object-contain relative z-10"
        />
      </div>
    </motion.button>
  );
...
function AppContent() {
  ...
  return (
    ...
    <div className="min-h-screen w-full bg-background text-foreground antialiased pb-16 lg:pb-0">
      ...
      <ForYouRail
        products={csvProducts.map(p => ({ id: p.id, name: p.name, category: p.category, price: p.price, image: resolveImageUrl(p.image) }))}
        onSelect={() => {
          const el = document.getElementById('products');
          ...
          <div className="mx-auto mt-6 max-w-5xl rounded-[30px] border border-primary/10 bg-card/95 p-6 shadow-[0_24px_70px_rgba(124,58,237,0.10)] backdrop-blur sm:p-8 lg:mt-10">
            ...
            <div className="mx-auto grid max-w-2xl grid-cols-2 gap-4">
              {[
                ...
                { value: csvProducts.length.toLocaleString(), label: 'Catalogue Products' },
              ].map((stat) => (
...
export default function App() {
  ...
  // Developer subdomain (or ?console=developer for local testing): render ONLY
  // the isolated Developer Console. The preview mounts App.tsx directly, so this
  // override lives here too — not just in AppRouter.
  if (isDeveloperHost()) {
    return (
