...
import { getBrowserStorage, IS_WEB } from './demo-mode';
...
export function markSessionStart() {
  ...
  try {
    ...
    storage.setItem(SESSION_START_KEY, now);
    storage.setItem(SESSION_ACTIVE_KEY, now);
  } catch { /* ignore */ }
}

/** Bump the sliding-window timestamp. Call on auth-state restore + activity. */
export function bumpSessionActivity() {
  try { getBrowserStorage()?.setItem(SESSION_ACTIVE_KEY, String(Date.now())); } catch { /* ignore */ }
}
...
export function clearSession() {
  ...
  try {
    ...
    storage?.removeItem(SESSION_FP_KEY);
    storage?.removeItem(SESSION_UID_KEY);
  } catch { /* ignore */ }
}
...
export async function bindSessionToDevice(uid: string) {
  ...
  try {
    ...
    storage.setItem(SESSION_FP_KEY, fp);
    storage.setItem(SESSION_UID_KEY, uid);
  } catch { /* ignore */ }
}
...
export async function isSessionDeviceValid(uid: string): Promise<boolean> {
  ...
  try {
    const storedUid = storage.getItem(SESSION_UID_KEY);
    const storedFp  = storage.getItem(SESSION_FP_KEY);
...
export function isSessionExpired(isStaff = false): boolean {
  ...
  try {
    const raw = storage.getItem(SESSION_ACTIVE_KEY) || storage.getItem(SESSION_START_KEY);
    if (!raw) return false;
...
function getOrCreateDeviceId(): string {
  const storage = getBrowserStorage();
  if (!storage) return 'native-device';
  try {
...
export async function computeDeviceFingerprint(): Promise<string> {
  if (!IS_WEB || typeof navigator === 'undefined' || typeof screen === 'undefined') return 'native-device';
  const parts = [
    navigator.userAgent || '',
    navigator.language || '',
...
export async function isSessionRevoked(uid: string): Promise<boolean> {
  ...
  try {
    ...
    const startRaw = storage.getItem(SESSION_START_KEY);
...
export async function recordSignInSuccess(uid: string, email: string) {
  ...
  await logSecurityEvent({
    ...
    userId: uid,
    email,
    userAgent: IS_WEB && typeof navigator !== 'undefined' ? navigator.userAgent : 'native',
