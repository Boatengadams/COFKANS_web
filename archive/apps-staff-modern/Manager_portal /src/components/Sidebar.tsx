import {
  ...
  BarChart2, Settings, HelpCircle, CheckSquare, Table2, Zap
} from 'lucide-react'
...
const NAV_MAIN = [
  ...
  { id: 'approvals',   label: 'Approvals',   icon: CheckSquare, badge: 23 },
...
interface Props { active: string; onNav: (id: string) => void; open?: boolean; onClose?: () => void }
...
export default function Sidebar({ active, onNav, open = false, onClose }: Props) {
  return (
    <>
    {open && <button className="staff-sidebar-backdrop" aria-label="Close navigation" onClick={onClose} />}
    <aside className={`staff-sidebar${open ? ' is-open' : ''}`} style={{
      width: 216,
      flexShrink: 0,
      background: '#0D1117',
      display: 'flex',
      ...
      <div style={{ padding: '10px 10px 4px' }}>
        <div style={{
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 12, fontWeight: 500, color: '#cbd5e1', lineHeight: 1.2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Head Office</div>
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.22)', marginTop: 1 }}>synced 2m ago</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '8px 10px 14px' }}>
        ...
        >
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ fontSize: 12, fontWeight: 500, color: '#cbd5e1', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', letterSpacing: '-0.01em' }}>Kwame Asante</div>
            <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.25)', letterSpacing: '0.05em', marginTop: 1 }}>GENERAL MANAGER</div>
          </div>
        </div>
      </div>
    </aside>
