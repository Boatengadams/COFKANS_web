...
function ContextMenu({ menu, onClose, onAction }: { menu: CtxMenu; onClose: () => void; onAction: (a: string, col: number, row: number) => void }) {
  useEffect(() => { const h = () => onClose(); window.addEventListener('mousedown', h); return () => window.removeEventListener('mousedown', h) }, [onClose])
  if (!menu) return null
  const items = [ { icon: Copy, label: 'Copy cell', action: 'copy' }, { icon: Scissors, label: 'Cut cell', action: 'cut' }, { icon: Plus, label: 'Insert row above', action: 'insAbove' }, { icon: Plus, label: 'Insert row below', action: 'insBelow' }, { icon: Trash2, label: 'Delete row', action: 'del' } ]
  return (
    <div onMouseDown={e => e.stopPropagation()} style={{ position: 'fixed', left: menu.x, top: menu.y, zIndex: 9999, background: '#fff', borderRadius: 8, minWidth: 168, boxShadow: '0 0 0 1px rgba(0,0,0,0.1), 0 8px 24px rgba(0,0,0,0.15)', padding: '4px' }}>
      {items.map(({ icon: Icon, label, action }, i) => (
        <button key={action} onMouseDown={() => { onAction(action, menu.col, menu.row); onClose() }} style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '7px 10px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, color: action === 'del' ? '#DC2626' : '#0A0F1E', textAlign: 'left', borderRadius: 5, transition: 'background 0.1s' }} onMouseEnter={e => (e.currentTarget.style.background = action === 'del' ? '#FEF2F2' : '#F8F9FB')} onMouseLeave={e => (e.currentTarget.style.background = 'none')}>
...
function useToasts() { const [toasts, setToasts] = useState<any[]>([]); const push = (message: string, tone = 'default') => { const id = Math.random().toString(36).slice(2); setToasts(t => [...t, { id, message, tone }]); setTimeout(() => setToasts(t => t.filter(x => x.id !== id)), 3000) }; return { toasts, push } }
...
export default function Spreadsheet() {
  ...
  // initialize defaults
  useEffect(() => { const init: Record<string, any> = {}; for (const s of SHEETS) init[s.id] = { rows: s.rows, headers: s.headers, label: s.label }; setRemoteSheets(init) }, [])

  // SINGLE document subscription: collection 'spreadsheet', doc 'sheets' containing nested map 'sheets'
  useEffect(() => {
    if (!canRead) { setLoading(false); return }
    setLoading(true)
    const d = doc(db, 'spreadsheet', 'sheets')
    const unsub = onSnapshot(d, snap => {
      if (snap.exists()) {
        const data = snap.data() as any
        const map = data.sheets ?? {}
        const normalized: Record<string, any> = {}
        for (const s of SHEETS) {
          const entry = map[s.id]
          normalized[s.id] = { rows: entry?.rows ?? s.rows, headers: entry?.headers ?? s.headers, label: entry?.label ?? s.label }
        }
        setRemoteSheets(normalized)
      }
      setLoading(false)
    }, err => { console.error('Spreadsheet snapshot error', err); setError('Failed to load spreadsheet from Firestore'); setLoading(false) })
    return () => unsub()
  }, [user, hasRole])
  ...
  const getCellValue = useCallback((col: number, row: number): CellValue => { const key = cellKey(col, row); if (key in gridOvr) return gridOvr[key]; return (row < baseRows.length && col < (baseRows[row]?.length ?? 0)) ? baseRows[row][col] : '' }, [gridOvr, baseRows])

  // merge overrides into the remote rows array
  const mergeRows = (sheetId: string) => {
    ...
    for (const k of keys) {
      ...
      if (rowNum >= rows.length) for (let i = rows.length; i <= rowNum; i++) rows.push([])
  ...
  // Persist into single doc: set nested field sheets.{sheetId}.rows etc
  const persistSheet = async (sheetId: string, rows: CellValue[][]) => {
    if (!canWrite) { push('You do not have permission to save changes (developer role required)', 'danger'); return }
    try {
      const payload: any = {
        updatedAt: serverTimestamp(),
      }
      payload[`sheets.${sheetId}.rows`] = rows
      payload[`sheets.${sheetId}.headers`] = headers
      payload[`sheets.${sheetId}.label`] = sheetMeta.label
      await setDoc(doc(db, 'spreadsheet', 'sheets'), payload, { merge: true })
      push('Saved', 'success')
    } catch (err) {
      console.error(err)
  ...
  const setCellValue = async (col: number, row: number, val: CellValue) => {
    ...
    setTimeout(async () => { const merged = mergeRows(activeSheet); await persistSheet(activeSheet, merged) }, 10)
  }

  const startEdit = (col: number, row: number, initial?: string) => { setEditing({ col, row }); setEditVal(initial ?? String(getCellValue(col, row))) }
  const commitEdit = useCallback(async () => { if (!editing) return; const num = Number(editVal); const value: CellValue = !isNaN(num) && editVal !== '' ? num : editVal; await setCellValue(editing.col, editing.row, value); setEditing(null) }, [editing, editVal, activeSheet])

  const sortedRows = (() => { let rows = baseRows.map((_, i) => i); if (sortCol !== null && sortDir) rows = [...rows].sort((a, b) => { const va = getCellValue(sortCol, a); const vb = getCellValue(sortCol, b); const na = typeof va === 'number', nb = typeof vb === 'number'; let cmp = na && nb ? (va as number) - (vb as number) : String(va).localeCompare(String(vb)); return sortDir === 'desc' ? -cmp : cmp }); if (search) rows = rows.filter(r => headers.some((_, c) => String(getCellValue(c, r)).toLowerCase().includes(search.toLowerCase()))); if (filter && filter.size > 0) rows = rows.filter(r => filterCol !== null && filter.has(String(getCellValue(filterCol, r)))); const total = rows.length + EXTRA_ROWS; for (let i = rows.length; i < total; i++) rows.push(baseRows.length + (i - rows.length)); return rows })()

  const handleKeyDown = (e: React.KeyboardEvent) => { if (!selected) return; const { col, row } = selected; if (e.key === 'Enter') { if (editing) { commitEdit(); setSelected({ col, row: row + 1 }) } else startEdit(col, row) } else if (e.key === 'Escape') { setEditing(null) } else if (e.key === 'Tab') { e.preventDefault(); commitEdit(); setSelected({ col: (col + 1) % headers.length, row }) } else if (e.key === 'Delete' || e.key === 'Backspace') { if (!editing) { setCellValue(col, row, ''); e.preventDefault() } } else if (e.key === 'ArrowDown') { e.preventDefault(); if (editing) commitEdit(); setSelected({ col, row: row + 1 }) } else if (e.key === 'ArrowUp') { e.preventDefault(); if (editing) commitEdit(); setSelected({ col, row: Math.max(0, row - 1) }) } else if (e.key === 'ArrowRight') { e.preventDefault(); if (!editing) setSelected({ col: Math.min(headers.length - 1, col + 1), row }) } else if (e.key === 'ArrowLeft') { e.preventDefault(); if (!editing) setSelected({ col: Math.max(0, col - 1), row }) } else if (e.key.length === 1 && !editing && !e.ctrlKey && !e.metaKey) { startEdit(col, row, e.key) } else if ((e.ctrlKey || e.metaKey) && e.key === 'c') { const val = String(getCellValue(col, row)); navigator.clipboard?.writeText(val).catch(() => {}); setClipboard(val) } else if ((e.ctrlKey || e.metaKey) && e.key === 'v') { if (!editing) setCellValue(col, row, clipboard) } }

  useEffect(() => { if (editing && inputRef.current) inputRef.current.select() }, [editing])

  const handleCtxAction = (action: string, col: number, row: number) => { if (action === 'copy') setClipboard(String(getCellValue(col, row))); if (action === 'cut') { setClipboard(String(getCellValue(col, row))); setCellValue(col, row, '') } if (action === 'del') { headers.forEach((_, c) => setCellValue(c, row, '')) } if (action === 'insBelow') { if (canWrite) { const merged = mergeRows(activeSheet); merged.splice(row + 1, 0, []); persistSheet(activeSheet, merged) } else { setExtraRows(prev => ({ ...prev, [activeSheet]: [...(prev[activeSheet] || []), []] })) } }
  }
  ...
  const isInRange = (col: number, row: number) => { if (!selRange) return false; const minC = Math.min(selRange.start.col, selRange.end.col); const maxC = Math.max(selRange.start.col, selRange.end.col); const minR = Math.min(selRange.start.row, selRange.end.row); const maxR = Math.max(selRange.start.row, selRange.end.row); return col >= minC && col <= maxC && row >= minR && row <= maxR }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 50px)', overflow: 'hidden', background: '#fff' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 14px', height: 42, flexShrink: 0, background: '#FAFAFA', borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
        ...
        {[ { icon: Copy, label: 'Copy' }, { icon: Scissors, label: 'Cut' }, { icon: Trash2, label: 'Delete Row' }, { icon: Plus, label: 'Add Row' } ].map(({ icon: Icon, label }) => (
          <button key={label} title={label} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 8px', borderRadius: 5, border: 'none', background: 'none', cursor: 'pointer', fontSize: 11, color: '#6B7280' }} onClick={() => { if (label === 'Copy' && selected) setClipboard(String(getCellValue(selected.col, selected.row))); if (label === 'Add Row') { if (canWrite) { const merged = mergeRows(activeSheet); merged.push([]); persistSheet(activeSheet, merged) } else { setExtraRows(prev => ({ ...prev, [activeSheet]: [...(prev[activeSheet] || []), []] })) } } if (label === 'Delete Row' && selected) headers.forEach((_, c) => setCellValue(c, selected.row, '')) }}>
            <Icon size={12} />
          </button>
        ))}
        <div style={{ flex: 1 }} />
        ...
        <table style={{ borderCollapse: 'collapse', tableLayout: 'fixed', minWidth: '100%' }}>
          ...
          <tbody>
            {sortedRows.map((rowIdx, displayIdx) => {
              ...
              const isEmpty = rowIdx >= baseRows.length && !Object.keys(gridOvr).some(k => { const r = parseInt(k.slice(1)) - 1; return r === rowIdx })
              return (
                <tr key={`${rowIdx}-${displayIdx}`} style={{ background: isEven ? '#FAFAFA' : '#fff' }}>
                  ...
                  {headers.map((_, colIdx) => {
                    ...
                    return (
                      <td key={colIdx} style={{ height: 26, padding: isEditing ? 0 : '0 10px', borderRight: '1px solid #F3F4F6', borderBottom: '1px solid #F3F4F6', outline: isSelected ? `2px solid ${sheet.color}` : inRange ? `1px solid ${sheet.color}60` : 'none', outlineOffset: -1, background: isSelected ? `${sheet.color}08` : inRange ? `${sheet.color}05` : undefined, fontSize: 12, color: '#0A0F1E', fontFamily: isNum ? 'JetBrains Mono, monospace' : 'Inter, sans-serif', textAlign: isNum ? 'right' : 'left', cursor: 'cell', userSelect: 'none', whiteSpace: 'nowrap', overflow: 'hidden', verticalAlign: 'middle' }} onClick={e => { if (editing && !(editing.col === colIdx && editing.row === rowIdx)) commitEdit(); if (e.shiftKey && selected) { setSelRange({ start: selected, end: { col: colIdx, row: rowIdx } }) } else { setSelRange(null); setSelected({ col: colIdx, row: rowIdx }) } gridRef.current?.focus() }} onDoubleClick={() => startEdit(colIdx, rowIdx)} onContextMenu={e => { e.preventDefault(); setSelected({ col: colIdx, row: rowIdx }); setCtxMenu({ x: e.clientX, y: e.clientY, col: colIdx, row: rowIdx }) }}>
                        {isEditing ? (
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 34, flexShrink: 0, borderTop: '1px solid rgba(0,0,0,0.07)', background: '#F8F9FB' }}>
        <div style={{ display: 'flex', alignItems: 'center', height: '100%', gap: 2, paddingLeft: 8 }}>
          {SHEETS.map(s => { const Icon = s.icon; const isActive = s.id === activeSheet; return (
