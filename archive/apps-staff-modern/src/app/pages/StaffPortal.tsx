...
import { PermissionProvider } from '../modules/branches/permissions';
...
const DeveloperConsole = lazy(() =>
  import('./DeveloperConsole').then(m => ({ default: m.DeveloperConsole }))
);
...
const ROLE_META: Record<Exclude<ResolvedRole['kind'], 'none'>, { label: string; icon: any }> = {
  ...
  warehouse:        { label: 'Warehouse',       icon: Package },
  accountant:       { label: 'Accountant',      icon: Package },
  hr:               { label: 'HR',              icon: Package },
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        else if (r === 'warehouse' || r === 'accountant' || r === 'hr' || r === 'procurement' || r === 'marketing') {
          setRole({ kind: r });
        }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main>
        <Suspense fallback={<FullScreenSpinner />}>
          ...
          {(['warehouse', 'accountant', 'hr', 'procurement', 'marketing'] as const).includes(role.kind as never) && (
            <PermissionProvider role={role.kind as 'warehouse' | 'accountant' | 'hr' | 'procurement' | 'marketing'}>
              <RoleDashboard branchId="company" />
            </PermissionProvider>
          )}
        </Suspense>
      </main>
