...
import {getPublicEnv, IS_WEB} from '@/lib/demo-mode';

export function VersionPanel() {
  const rows: [string, string][] = [
    ['Build SHA', getPublicEnv('BUILD_SHA') || 'dev'],
    ['Build time', getPublicEnv('BUILD_AT') || 'dev'],
    ['Firebase project', getApp().options.projectId ?? '—'],
    ['Auth domain', getApp().options.authDomain ?? '—'],
    ['Hostname', IS_WEB && typeof window !== 'undefined' ? window.location.hostname : '—'],
    ['User agent', IS_WEB && typeof navigator !== 'undefined' ? navigator.userAgent : '—'],
  ];
