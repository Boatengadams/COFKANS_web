...
import { StockTransferPanel } from '../components/portal/StockTransferPanel';
...
type PortalTab = 'deliveries' | 'transfers';

export function DriverPortal({ onBack }: DriverPortalProps) {
  const { user } = useFirebaseAuth();
  const driverId = user?.uid;
  const [portalTab, setPortalTab] = useState<PortalTab>('deliveries');
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-card border-b-2 border-border">
        <div className="max-w-7xl mx-auto px-6 flex gap-1 overflow-x-auto py-2">
          {([
            { id: 'deliveries' as PortalTab, label: 'Customer Deliveries', icon: Truck },
            { id: 'transfers'  as PortalTab, label: 'Branch Transfers',    icon: ArrowLeftRight },
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold whitespace-nowrap transition-colors ${
                portalTab === id ? 'bg-primary text-white' : 'bg-muted text-muted-foreground hover:bg-muted/80'
              }`}
            >
      ...
      {portalTab === 'transfers' && (
        <div className="max-w-7xl mx-auto px-6 py-8">
          <StockTransferPanel mode="driver" driverUid={driverId} />
        </div>
      )}

      {/* Deliveries tab (existing UI) */}
      {portalTab === 'deliveries' && (
      <div className="max-w-7xl mx-auto px-6 py-8">
