...
import { Search, Bell, ChevronRight, User, LogOut, Settings } from 'lucide-react'
...
interface Props { page: string; onSignOut?: () => void }

export default function TopBar({ page, onSignOut }: Props) {
  ...
  useEffect(() => {
    let active = true
    if (!firebaseUser) { setStaff(null); return () => { active = false } }
    getStaffAccount(firebaseUser.uid).then(account => { if (active) setStaff(account) })
    return () => { active = false }
  }, [firebaseUser])

  const firebaseDisplayName =
    firebaseUser && 'displayName' in firebaseUser && typeof firebaseUser.displayName === 'string'
      ? firebaseUser.displayName
      : ''
  const displayName = staff?.displayName || staff?.email || user?.displayName || firebaseDisplayName || ''
  const email = staff?.email || firebaseUser?.email || user?.email || ''
  ...
  return (
    <header style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: '#94A3B8' }}>
        ...
        <span style={{ color: '#0A0F1E', fontWeight: 500, fontSize: 13 }}>{PAGE_META[page] || page}</span>
      </div>
      ...
      <div style={{ position: 'relative' }}>
        ...
        >
          <div style={{ width: 24, height: 24, borderRadius: '50%', background: 'linear-gradient(135deg, #16A34A, #15803D)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: '#fff' }}>{initials}</div>
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#0A0F1E', lineHeight: 1.3, letterSpacing: '-0.01em' }}>{displayName.split(' ').slice(0, 2).join(' ')}</div>
            <div style={{ fontSize: 9, color: '#94A3B8', lineHeight: 1.2 }}>{staff?.role || 'Staff account'}</div>
          </div>
        </button>
        ...
        {profileOpen && (
          <div style={{ position: 'absolute', right: 0, top: 'calc(100% + 6px)', width: 188, background: '#fff', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 10, boxShadow: '0 4px 24px rgba(0,0,0,0.10)', overflow: 'hidden', zIndex: 50 }}>
            <div style={{ padding: '12px 14px', borderBottom: '1px solid #F3F4F6' }}>
              <div style={{ fontWeight: 600, fontSize: 13, color: '#0A0F1E', letterSpacing: '-0.01em' }}>{displayName}</div>
              <div style={{ fontSize: 11, color: '#94A3B8', marginTop: 1 }}>{email}</div>
            </div>
            {[{ icon: User, label: 'View Profile' }, { icon: Settings, label: 'Settings' }, { icon: LogOut, label: 'Sign Out', danger: true, onClick: handleSignOut }].map(({ icon: Icon, label, danger, onClick }) => (
              <button key={label} onClick={() => { onClick?.(); setProfileOpen(false); }}
