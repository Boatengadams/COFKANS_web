...
import {
  ...
  Clock, RefreshCw, UserPlus, X, ArrowLeftRight, AlertTriangle,
} from 'lucide-react';
import { db } from '@/lib/firebase';
import { DEMO_MODE } from '@/lib/demo-mode';
import { useBranches } from '@/lib/branches';
import {
  STATUS_LABEL, statusColor, statusLabel, nextStatus, updateBranchOrderStatus,
  type BranchOrder, type BranchOrderStatus,
} from '@/lib/branch-orders';
import { ShowroomInventoryPanel } from './ShowroomInventoryPanel';
...
type Tab = 'overview' | 'queue' | 'products' | 'stock' | 'transfers' | 'alerts' | 'appointments' | 'calls' | 'walkin';

function dateValue(value: unknown): number {
  if (typeof (value as { toMillis?: () => number })?.toMillis === 'function') return (value as { toMillis: () => number }).toMillis();
  if (typeof (value as { seconds?: number })?.seconds === 'number') return (value as { seconds: number }).seconds * 1000;
...
export function FrontDeskDashboard() {
  ...
  useEffect(() => {
    ...
    let unsub = () => {};
    const bind = (ordered: boolean) => {
      const source = ordered
        ? query(collection(db, 'orders'), where('scheduledDate', '==', todayIso), orderBy('createdAt', 'desc'))
        : query(collection(db, 'orders'), where('scheduledDate', '==', todayIso));
      unsub = onSnapshot(source, snap => {
        const next = snap.docs.map(d => ({ id: d.id, ...(d.data() as Omit<BranchOrder, 'id'>) }));
        if (!ordered) next.sort((a, b) => dateValue(b.createdAt) - dateValue(a.createdAt));
        setToday(next);
        setLoading(false);
      }, () => {
        if (ordered) bind(false); else setLoading(false);
      });
    };
    bind(true);
    return () => unsub();
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      {tab === 'overview' && (
        <div className="space-y-6">
          <BranchModuleLauncher variant="frontdesk" />
          <Overview byBranch={byBranch} todayIso={todayIso} branches={branches} />
        </div>
      )}
      ...
      {tab === 'queue' && (
        <Queue
          ...
          onAdvance={advance} branches={branches}
        />
      )}
      ...
      {tab === 'products'  && (
        <div className="erp-theme">
          <ProductManagementPanel
            ...
            subtitle="Add showroom products and update prices — removals are handled by the Developer."
          />
        </div>
      )}
      {tab === 'stock'     && <ShowroomInventoryPanel />}
      {tab === 'transfers' && <StockTransferPanel mode="frontdesk" />}
      {tab === 'alerts'    && <StockAlertsPanel />}

      {tab === 'appointments' && <Appointments orders={today} />}

      {tab === 'calls' && <CallSheet orders={today} todayIso={todayIso} branches={branches} />}
...
/* ---------- Overview ---------- */
function Overview({ byBranch, todayIso, branches }: { byBranch: Map<string, BranchOrder[]>; todayIso: string; branches: { slug: string; name: string }[] }) {
  return (
          ...
          {[...byBranch.entries()].map(([slug, list]) => (
            ...
            >
              ...
              <ul className="space-y-1.5">
                ...
                {list.length > 6 && <li className="text-[11px] text-muted-foreground pt-0.5">+{list.length - 6} more</li>}
              </ul>
            </motion.div>
          ))}
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2.5 rounded-xl bg-card border-2 border-border text-sm focus:outline-none focus:border-primary"
          />
        </div>
        <select value={branchFilter} onChange={e => setBranchFilter(e.target.value)}
          className="px-3 py-2.5 rounded-xl bg-card border-2 border-border text-sm font-semibold">
          ...
          {orders.map(o => {
            ...
            return (
              <motion.div
                ...
                className="bg-card border-2 border-border rounded-xl p-3 flex items-center gap-3"
              >
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${o.fulfillmentType === 'pickup' ? 'bg-amber-500/10 text-amber-500' : 'bg-violet-500/10 text-violet-500'}`}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[11px] text-muted-foreground truncate">
                    ...
                    {o.itemCount != null && <> · {o.itemCount} item{o.itemCount === 1 ? '' : 's'}</>}
                  </div>
                </div>
                ...
                {o.shippingAddress?.phone && (
                  <a href={`tel:${o.shippingAddress.phone}`} className="w-9 h-9 rounded-lg border-2 border-border hover:bg-muted flex items-center justify-center shrink-0" title="Call">
                ...
                {ns ? (
                  ...
                  <span className="hidden sm:inline-flex items-center gap-1 px-3 py-2 text-xs font-bold text-emerald-600 shrink-0">
...
function Column({ title, icon: Icon, tone, list }: { title: string; icon: typeof Store; tone: string; list: BranchOrder[] }) {
  return (
    <section className="bg-card border-2 border-border rounded-2xl p-5">
      <h3 className="font-bold mb-3 flex items-center gap-2"><Icon className={`w-4 h-4 text-${tone}-500`} /> {title} <span className="text-muted-foreground font-normal">· {list.length}</span></h3>
      {list.length === 0 ? (
        <EmptyState icon={Clock} text={`No ${title.toLowerCase()} scheduled.`} />
      ) : (
        <ul className="space-y-2">
          {list.map(o => (
            <li key={o.id} className="bg-background border border-border rounded-xl p-3 flex items-center justify-between gap-2">
...
/* ---------- Call sheet ---------- */
function CallSheet({ orders, todayIso, branches }: { orders: BranchOrder[]; todayIso: string; branches: { slug: string; name: string }[] }) {
  return (
    <section className="bg-card border-2 border-border rounded-2xl p-5">
            ...
            <tbody>
              {orders.map(o => (
                <tr key={o.id} className="border-b border-border/60">
                  ...
                  <td className="pr-3 text-muted-foreground">{branches.find(b => b.slug === o.branchSlug)?.name ?? '—'}</td>
                  ...
                  <td className="text-muted-foreground max-w-[14rem]">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <section className="bg-card border-2 border-border rounded-2xl p-5">
        ...
        <div className="space-y-3">
          <Field label="Customer name"><input value={name} onChange={e => setName(e.target.value)} className="field" placeholder="e.g. Kwame Mensah" /></Field>
          <Field label="Phone"><input value={phone} onChange={e => setPhone(e.target.value)} className="field" placeholder="024 000 0000" /></Field>
          <Field label="What do they need?"><textarea value={reason} onChange={e => setReason(e.target.value)} rows={3} className="field resize-none" placeholder="e.g. Quote for a 3kVA inverter + install" /></Field>
        ...
        <style>{`.field{width:100%;padding:0.625rem 0.75rem;border-radius:0.75rem;background:var(--background);border:2px solid var(--border);font-size:0.875rem;outline:none}.field:focus{border-color:var(--primary)}`}</style>
      </section>

      <section className="bg-card border-2 border-border rounded-2xl p-5">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-bold flex items-center gap-2"><Users className="w-4 h-4 text-primary" /> Today's walk-ins <span className="text-muted-foreground font-normal">· {log.length}</span></h3>
          {log.length > 0 && (
            <button onClick={() => setLog([])} className="text-xs text-muted-foreground hover:text-foreground inline-flex items-center gap-1"><RefreshCw className="w-3 h-3" /> Clear</button>
          )}
        </div>
        {log.length === 0 ? (
          ...
          <ul className="space-y-2">
            {log.map(w => (
              <li key={w.id} className="bg-background border border-border rounded-xl p-3 flex items-start justify-between gap-2">
                <div className="min-w-0">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button onClick={() => setLog(prev => prev.filter(x => x.id !== w.id))} className="text-muted-foreground hover:text-foreground"><X className="w-3.5 h-3.5" /></button>
                </div>
              </li>
            ))}
          </ul>
        )}
        <p className="mt-3 text-[11px] text-muted-foreground">This log is for your shift. Convert serious enquiries into orders from the POS.</p>
      </section>
    </div>
  );
}
