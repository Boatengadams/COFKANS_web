...
const greet = (name: string, role: ChatUserRole): Message => {
  ...
  const text = (() => {
    switch (role) {
      ...
      case 'guest':
        return `Hi there 👋 I'm Ama, your Cofkans concierge. I can help you find the right lighting, solar, inverter or smart-home setup, check delivery, or book an install — and I'm here 24/7. What are you working on today? (Tip: sign in for personalised order help.)`;
      case 'customer':
      default:
        return `Hi ${first} 👋 I'm Ama, your personal Cofkans concierge. Ask me about products, pricing, your orders, delivery or installation — or let me recommend the perfect fit for your space. How can I help today?`;
    }
  })();
  const isStaff = role === 'driver' || role === 'technician' || role === 'support_agent' || role === 'admin';
  return { id: uid(), role: 'ai', authorName: isStaff ? 'Cofkans Assistant' : 'Ama · Concierge', text, at: Date.now() };
};
