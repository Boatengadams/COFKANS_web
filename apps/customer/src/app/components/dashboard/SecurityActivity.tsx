  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        let snap;
        try {
          snap = await getDocs(query(collection(db, 'securityEvents'), where('userId', '==', firebaseUser.uid), orderBy('timestamp', 'desc'), limit(20)));
        } catch {
          snap = await getDocs(query(collection(db, 'securityEvents'), where('userId', '==', firebaseUser.uid), limit(20)));
        }
        ...
        next.sort((a, b) => (b.timestamp?.getTime() ?? 0) - (a.timestamp?.getTime() ?? 0));
        setEvents(next);
      } catch (e) {
