      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 z-50 flex h-[100dvh] w-full max-w-md flex-col border-l border-border bg-background shadow-2xl sm:h-full"
          >
            {/* Header */}
            <div className="flex items-center justify-between border-b border-border p-4 pt-[max(1rem,env(safe-area-inset-top))] sm:p-6">
              ...
              <button
                onClick={onClose}
                className="flex min-h-11 min-w-11 items-center justify-center rounded-lg hover:bg-muted transition-colors"
              >
                <X className="w-6 h-6" />
              </button>
            </div>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="w-9 h-9 rounded-lg bg-muted hover:bg-muted/80 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center transition-colors"
                            >
            ...
            {cart && cart.items.length > 0 && (
              <div className="mobile-bottom-safe border-t border-border bg-muted/30 p-4 sm:p-6">
                <div className="space-y-3 mb-4">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">Subtotal</span>
                    <span className="font-semibold">GH₵{(cart?.subtotal ?? 0).toFixed(2)}</span>
                  </div>

                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">Tax (12.5%)</span>
                    <span className="font-semibold">GH₵{(cart?.taxAmount ?? 0).toFixed(2)}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-semibold">
                      {(cart?.shippingAmount ?? 0) === 0 ? (
                        ...
                        `GH₵${(cart?.shippingAmount ?? 0).toFixed(2)}`
                      )}
                    </span>
                  </div>
                  ...
                  {(cart?.subtotal ?? 0) < 500 && (
                    <div className="text-xs text-muted-foreground bg-primary/10 p-2 rounded-lg">
                      Add GH₵{(500 - (cart?.subtotal ?? 0)).toFixed(2)} more for free shipping
                    </div>
                  )}

                  <div className="pt-3 border-t border-border flex items-center justify-between">
                    <span className="font-bold text-lg">Total</span>
                    <span className="font-bold text-2xl text-primary">GH₵{(cart?.total ?? 0).toFixed(2)}</span>
                  </div>
                </div>
