...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    plugins: [
      react(),
      tailwindcss(),
      figmaSiteConfiguration(siteConfiguration, publicSiteUrl),
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration, publicSiteUrl: string): Plugin {
  ...
  const robotsTxt = publicSiteUrl
    ? `User-agent: *\nAllow: /\nSitemap: ${publicSiteUrl}/sitemap.xml\n`
    : 'User-agent: *\nDisallow: /\n'
  const sitemapPaths = ['/', '/about', '/branches', '/contact', '/faq', '/installation', '/privacy', '/shipping', '/support', '/terms', '/warranty', '/careers']
  ...
  return {
    ...
    generateBundle() {
      ...
      if (publicSiteUrl) {
        const urls = sitemapPaths.map((pathname) => `  <url><loc>${publicSiteUrl}${pathname}</loc></url>`).join('\n')
        this.emitFile({
          type: 'asset',
